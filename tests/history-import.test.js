...
assert.deepEqual(normalized.balanceUpdates,{food:15694});
const result=api.mergeHistory(original,records,normalized.balanceUpdates);
assert.equal(JSON.stringify(original),snapshot);
assert.deepEqual(result.state.balances,{food:15694,pocket:37143});
assert.equal(result.fresh.length,1);
assert.equal(result.skipped,1);
assert.equal(result.total,1060);
assert.equal(result.state.transactions.length,2);

const repeated=api.mergeHistory(result.state,records,{food:999});
assert.equal(repeated.fresh.length,0);
assert.equal(repeated.state.balances.food,15694);
assert.equal(repeated.state.transactions.length,2);
...
assert.throws(()=>api.normalizePayload({...payload,balanceUpdates:{food:-1}}));
