...
assert.equal(exported.exportSpecification.version,'1.1.0');
...
assert.equal(exported.allocationHistory.length,1);
assert.equal(exported.allocationHistory[0].items.length,2);
assert.equal(exported.allocationHistory[0].items[1].name,'食費・日用品');
...
assert.ok(csv.includes('allocation_item'));
const clipboard=api.toClipboardText(exported);
assert.ok(clipboard.includes('現在残高と全期間の履歴データ'));
assert.ok(clipboard.includes('【支出履歴・全期間 2件】'));
assert.ok(clipboard.includes('2026-07-20｜500円｜お小遣い｜確定｜=危険な式｜対象期間：期間外'));
assert.ok(clipboard.includes('【今期集計】'));
assert.ok(clipboard.includes('確定支出：1,200円（1件）'));
assert.equal(clipboard.includes('【お金コンパス共有データ（JSON）】'),false);
assert.equal(clipboard.includes('do-not-export'),false);
assert.ok(api.shareSummary(exported).includes('支出全履歴 2件'));
